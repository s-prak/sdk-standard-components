...
// the JWS signature algorithms to use. Note that Mojaloop spec requires RS256 at present
...
const ALL_SIGNATURE_ALGORITHMS = [...SIGNATURE_ALGORITHMS, ...PQ_SIGNATURE_ALGORITHMS];
...
class JwsValidator {
    ...
    validate (request) {
        try {
            ...
            const token = `${protectedHeader}.${payloadBase64}.${signature}`;
            this.logger.debug(`JWS token to verify: ${token}, using public key: ${pubKey}`);

            // Decode protected header to determine algorithm
            const decodedProtectedHeaderJson = JSON.parse(base64url.decode(protectedHeader));

            if (PQ_SIGNATURE_ALGORITHMS.includes(decodedProtectedHeaderJson.alg)) {
                // PQC verification path using Node.js crypto (ML-DSA family)
                const signingInput = Buffer.from(`${protectedHeader}.${payloadBase64}`, 'utf8');
                const signatureBuffer = base64url.toBuffer(signature);

                const isValid = crypto.verify(null, signingInput, pubKey, signatureBuffer);
                if (!isValid) {
                    throw new Error(`Invalid JWS signature for algorithm '${decodedProtectedHeaderJson.alg}'`);
                }

                // check protected header has all required fields and matches actual incoming headers
                this._validateProtectedHeader(headers, decodedProtectedHeaderJson);

                this.logger.debug('JWS verify result for request (PQC path):', { request, header: decodedProtectedHeaderJson });
                return true;
            }

            // Classical verification path using jsonwebtoken for RS256 / ES256
            const result = jwt.verify(token, pubKey, {
                complete: true,
                algorithms: SIGNATURE_ALGORITHMS // only allow our classical signature algorithms
            });
    ...
    // check alg is present and is one of the permitted values
        ...
        if (!ALL_SIGNATURE_ALGORITHMS.includes(decodedProtectedHeader.alg)) {
            throw new Error(`Invalid protected header alg '${decodedProtectedHeader.alg}' should be '${ALL_SIGNATURE_ALGORITHMS.join(' or ')}'`);
        }
